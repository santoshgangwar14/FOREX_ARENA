...
export default function SidebarLayout({
  ...
  const navItems = [
    ...
    { name: 'Withdrawal', path: '/withdrawal', icon: ArrowDownToLine },
    { name: 'Economic Calendar', path: '/calendar', icon: CalendarDays },
  ...
  const supportItems = [
    ...
    { name: 'Raise Complaint', path: '/complaints', icon: AlertTriangle },
  ];
    ...
    <div className="min-h-screen bg-[#050505] text-[#f0f0f0] flex flex-col md:flex-row font-sans">
      <header className="md:hidden flex items-center justify-between px-5 py-4 bg-[#080808] border-b border-[#1a1a1a] sticky top-0 z-50">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 bg-gradient-to-tr from-[#D4AF37] to-[#F9E29B] rounded-lg flex items-center justify-center">
      ...
      >
        ...
        <nav className="flex-1 px-4 space-y-1 text-sm overflow-y-auto">
            ...
            return (
              <NavLink
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-r-md text-sm transition-all duration-200 ${
                  isActive
                    ? 'bg-[#121212] border-l-2 border-[#D4AF37] text-white font-semibold pl-3.5'
          ...
          <div className="pt-5 mt-4 border-t border-[#1a1a1a]">
            <p className="px-4 mb-2 text-[10px] uppercase tracking-[0.18em] text-gray-600 font-bold">
              Support
            </p>
        ...
        <div className="p-6 mt-auto border-t border-[#1a1a1a]">
          ...
          <button
            ...
            className="w-full py-2 text-xs font-medium text-red-400 hover:text-red-300 flex items-center justify-center gap-2 border border-red-900/30 rounded bg-red-950/10 transition-colors"
          >
            ...
            <div className="flex flex-col min-w-max">
              ...
              <span
                className={`text-sm font-bold font-mono ${
                  (wallet?.floatingPL || 0) >= 0
                    ? 'text-emerald-400'
                    : 'text-rose-500'
                }`}
              >
                {(wallet?.floatingPL || 0) >= 0 ? '+' : ''}
                {formatCurrency(wallet?.floatingPL)}
              </span>
            </div>
